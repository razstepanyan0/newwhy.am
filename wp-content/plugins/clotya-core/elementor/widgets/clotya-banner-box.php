...
class Clotya_Banner_Box_Widget extends Widget_Base {

    public function get_name() {
        return 'clotya-banner-box';
    }
	...
	protected function register_controls() {
		...
		$this->add_control( 'banner_type',
			[
				'label' => esc_html__( 'Banner Type', 'clotya' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'select-type',
				'options' => [
					'select-type' => esc_html__( 'Banner Type', 'clotya' ),
					'type1'	  => esc_html__( 'Type1', 'clotya' ),
					'type2'	  => esc_html__( 'Type2', 'clotya' ),
					'type3'	  => esc_html__( 'Type3', 'clotya' ),
				],
			]
		);	
	...
	protected function render() {
		...
		echo '<div class="klb-banner-box banner dark '.esc_attr($settings['vertical_type']).' content-left mobile-sec '.esc_attr($type).'">';
		echo '<div class="banner-content">';
		echo '<div class="banner-inner">';
		echo '<h4 class="entry-subtitle">'.esc_html($settings['subtitle']).'</h4>';
		echo '<h2 class="entry-title '.esc_attr($fontsize).'">'.esc_html($settings['title']).'</h2>';
		echo '<div class="entry-description">';
		echo '<p>'.esc_html($settings['desc']).'</p>';
		echo '</div><!-- entry-description -->';
		if($settings['btn_title']){
			echo '<a href="'.esc_url($settings['btn_link']['url']).'" class="btn link" '.esc_attr($target.$nofollow).'>'.esc_html($settings['btn_title']).' <i class="klbth-icon-right-arrow"></i></a>';
		}
