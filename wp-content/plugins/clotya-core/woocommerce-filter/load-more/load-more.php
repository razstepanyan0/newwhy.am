...
if(get_theme_mod('clotya_paginate_type') == 'loadmore' || clotya_ft() == 'load-more'){
	function clotya_load_more_button(){
		echo '<nav class="woocommerce-pagination klb-load-more">
                <ul class="page-numbers">
					<div class="button light">'.esc_html__('Load More','clotya-core').'</div>
				</ul>
			  </nav>';
	}

	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
	add_action( 'woocommerce_after_shop_loop', 'clotya_load_more_button', 10 );
}
...
if(get_theme_mod('clotya_paginate_type') == 'infinite' || clotya_ft() == 'infinite'){
	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
}
...
if(get_theme_mod('clotya_ajax_on_shop',0) == '1'){
	add_action('woocommerce_before_main_content','clotya_loadmore_parameters',40);
	function clotya_loadmore_parameters() {
	...
	<script type="text/javascript">
		var loadmore = {
			...
			"no_more_products":"<?php echo esc_html_e('No More Products','clotya-core'); ?>",
...
## Quick View Scripts
*************************************************/
if(get_theme_mod('clotya_paginate_type') == 'loadmore' || get_theme_mod('clotya_paginate_type') == 'infinite' || clotya_ft() == 'load-more' || clotya_ft() == 'infinite'){
	if(get_theme_mod('clotya_ajax_on_shop',0) == '1'){
		function clotya_load_more_scripts() {
			if(is_shop() || is_tax()){
				if(get_theme_mod('clotya_paginate_type') == 'infinite' || clotya_ft() == 'infinite'){
					wp_enqueue_script( 'clotya-load-more',  plugins_url( 'js/infinite-scroll.js', __FILE__ ), true );
				} elseif(get_theme_mod('clotya_paginate_type') == 'loadmore' || clotya_ft() == 'load-more') {
					wp_enqueue_script( 'clotya-load-more',  plugins_url( 'js/load_more.js', __FILE__ ), true );
				}
			}
		}
		...
		function clotya_load_more_scripts() {
			if(is_shop() || is_tax()){
				...
				wp_localize_script( 'clotya-load-more', 'loadmore', array(
					...
					'no_more_products' => esc_html__('No More Products','clotya-core'),
	...
	add_action( 'wp_enqueue_scripts', 'clotya_load_more_scripts' );
}
...
add_action( 'wp_ajax_nopriv_load_more', 'clotya_load_more_callback' );
add_action( 'wp_ajax_load_more', 'clotya_load_more_callback' );
function clotya_load_more_callback() {
